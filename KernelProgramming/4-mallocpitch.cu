...
__global__ void simpleInit2D(T *buffer, T value, int cols, int rows, size_t pitch) {
    int col = blockIdx.x * blockDim.x + threadIdx.x;  // compute coordinates
    int row = blockIdx.y * blockDim.y + threadIdx.y;  // compute coordinates
    if(col < cols && row < rows) {  // check boundaries
