...
__global__ void add(int *a, int *b, int *c, int N, int M) {
    int i = blockIdx.y * blockDim.y + threadIdx.y;  // compute row the coordinates of the value
    int j = blockIdx.x * blockDim.x + threadIdx.x;  // compute col the coordinates of the value
    if(i < M && j < M) {  // check boundaries
