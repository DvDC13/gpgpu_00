...
int main() {
  ...
  check2D<<<blocks, threads>>>(d_buffer, expectedValue, cols, rows, pitch);
  ...
  float* host_buffer = (float*) std::malloc(cols * rows * sizeof(float));
  //@@ Copy the buffer content from device to host
  //@@ use cudaMemcpy2D
  cudaMemcpy2D(host_buffer, pitch, d_buffer, pitch, cols * sizeof(float), rows, cudaMemcpyDeviceToHost);
  cudaCheckError();
